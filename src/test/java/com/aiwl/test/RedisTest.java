...
/**
 * @Description Redis 的一些API 测试类 有不少类型
 * @author jakiro
 * @version V1.0
 * @Date 2015年7月9日 上午10:46:23
 * @mail terrorbladeyang@gmail.com
 */
public class RedisTest {
	static Jedis jedis = new Jedis("114.251.37.21");
	/**
	...
	public static void Hello() {
		Jedis jedis = new Jedis("114.251.37.21");
		try {
...
//		Jedis jedis = RedisPool.getJedis();
		...
		System.out.println(p.lpush("asd", "aaa"));
		// System.out.println(p.spop("asd").get());
		List<Object> object = p.syncAndReturnAll();
...
//		RedisPool.returnResource(jedis);
	...
	public static void testMore() {
		...
		// INCRBY,HINCRBY等等，redis有了原子递增（atomic
		// increment），你可以放心的加上各种计数，用GETSET重置，或者是让它们过期。
...
//		Jedis jedis = RedisPool.getJedis();
	...
	/**
	 * [向Redis list压入ID而不是实际的数据] 在上面的例子里 ，我们将“对象”（此例中是简单消息）直接压入Redis
	 * list，但通常不应这么做，
	 * 由于对象可能被多次引用：例如在一个list中维护其时间顺序，在一个集合中保存它的类别，只要有必要，它还会出现在其他list中，等等。
	 * 让我们回到reddit.com的例子，将用户提交的链接（新闻）添加到list中，有更可靠的方法如下所示： $ redis-cli incr
	 * next.news.id (integer) 1 $ redis-cli set news:1:title "Redis is simple"
	 * OK $ redis-cli set news:1:url "http://code.google.com/p/redis" OK $
	 * redis-cli lpush submitted.news 1 OK
	 * 我们自增一个key，很容易得到一个独一无二的自增ID，然后通过此ID创建对象
	 * –为对象的每个字段设置一个key。最后将新对象的ID压入submitted.news list。
	 * 这只是牛刀小试。在命令参考文档中可以读到所有和list有关的命令
	 * 。你可以删除元素，旋转list，根据索引获取和设置元素，当然也可以用LLEN得到list的长度。
	 */
